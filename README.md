...
```bash
# Auto-detect NAS100 and download all 13 bar timeframes (100k bars each) + 500k ticks
cmd /c python download_nas100_mt5.py --timeframe all --type both --bars 100000 --ticks 500000 --update_raw

# Download a specific timeframe (e.g. 5-minute bars) and update training set
cmd /c python download_nas100_mt5.py --timeframe M5 --bars 50000 --update_raw

# Download tick records only (up to 500,000 live ticks)
cmd /c python download_nas100_mt5.py --type ticks --ticks 500000
```

### Run the Global Multi-Timeframe Pipeline

```bash
# Run concurrent evaluation using multi-threading (default 4 worker threads)
cmd /c python run_train_backtest.py --mode threads --workers 4

# Run concurrent evaluation using multi-processing
cmd /c python run_train_backtest.py --mode processes --workers 4

# Run sequentially (single-thread baseline mode)
cmd /c python run_train_backtest.py --mode sequential
```
